...
SELECT AVG(stipendio), STDEV(stipendio) FROM Persona GROUP BY strutturato
...
SELECT * FROM Persona AS p, AttivitaProgetto as ap WHERE p.id = ap.id AND (SELECT SUM(ap2.oreDurata) FROM AttivitaProgetto AS ap2 WHERE ap2.id = p.id) >= 20

-- 5. Quali sono i progetti la cui durata è superiore alla media delle durate di
-- tutti i progetti? Restituire nome dei progetti e loro durata in giorni.
